...
#define PRTE_PROC_STATE_KILLED_BY_CMD \
    (PRTE_PROC_STATE_ERROR + 1) /* process was killed by PRTE cmd */
...
#define PRTE_PROC_STATE_TERM_WO_SYNC \
    (PRTE_PROC_STATE_ERROR + 5) /* process exit'd w/o required sync */
#define PRTE_PROC_STATE_COMM_FAILED \
    (PRTE_PROC_STATE_ERROR + 6) /* process communication has failed */
#define PRTE_PROC_STATE_SENSOR_BOUND_EXCEEDED \
    (PRTE_PROC_STATE_ERROR + 7) /* process exceeded a sensor limit */
#define PRTE_PROC_STATE_CALLED_ABORT (PRTE_PROC_STATE_ERROR + 8) /* process called "errmgr.abort" \
                                                                  */
#define PRTE_PROC_STATE_HEARTBEAT_FAILED \
    (PRTE_PROC_STATE_ERROR + 9) /* heartbeat failed to arrive */
#define PRTE_PROC_STATE_MIGRATING                                                                 \
    (PRTE_PROC_STATE_ERROR + 10) /* process failed and is waiting for resources before restarting \
                                  */
#define PRTE_PROC_STATE_CANNOT_RESTART \
    (PRTE_PROC_STATE_ERROR + 11) /* process failed and cannot be restarted */
#define PRTE_PROC_STATE_TERM_NON_ZERO \
    (PRTE_PROC_STATE_ERROR + 12) /* process exited with a non-zero status, indicating abnormal */
#define PRTE_PROC_STATE_FAILED_TO_LAUNCH (PRTE_PROC_STATE_ERROR + 13) /* unable to launch process \
                                                                       */
#define PRTE_PROC_STATE_UNABLE_TO_SEND_MSG \
    (PRTE_PROC_STATE_ERROR + 14)                                   /* unable to send a message */
#define PRTE_PROC_STATE_LIFELINE_LOST (PRTE_PROC_STATE_ERROR + 15) /* connection to lifeline lost \
                                                                    */
#define PRTE_PROC_STATE_NO_PATH_TO_TARGET \
    (PRTE_PROC_STATE_ERROR + 16) /* no path for communicating to target peer */
#define PRTE_PROC_STATE_FAILED_TO_CONNECT \
    (PRTE_PROC_STATE_ERROR + 17) /* unable to connect to target peer */
...
#define PRTE_JOB_STATE_TERMINATED \
    31 /* all processes have terminated and job is no longer running */
...
#define PRTE_JOB_STATE_ABORTED \
    (PRTE_JOB_STATE_ERROR + 2) /* at least one process aborted, causing job to abort */
#define PRTE_JOB_STATE_FAILED_TO_START \
    (PRTE_JOB_STATE_ERROR + 3) /* at least one process failed to start */
#define PRTE_JOB_STATE_ABORTED_BY_SIG (PRTE_JOB_STATE_ERROR + 4) /* job was killed by a signal */
#define PRTE_JOB_STATE_ABORTED_WO_SYNC \
    (PRTE_JOB_STATE_ERROR + 5) /* job was aborted because proc exit'd w/o required sync */
#define PRTE_JOB_STATE_COMM_FAILED (PRTE_JOB_STATE_ERROR + 6) /* communication has failed */
#define PRTE_JOB_STATE_SENSOR_BOUND_EXCEEDED \
    (PRTE_JOB_STATE_ERROR + 7) /* job had a process that exceeded a sensor limit */
#define PRTE_JOB_STATE_CALLED_ABORT \
    (PRTE_JOB_STATE_ERROR + 8) /* at least one process called "errmgr.abort" */
#define PRTE_JOB_STATE_HEARTBEAT_FAILED (PRTE_JOB_STATE_ERROR + 9) /* heartbeat failed to arrive \
                                                                    */
#define PRTE_JOB_STATE_NEVER_LAUNCHED                                            \
    (PRTE_JOB_STATE_ERROR + 10) /* the job never even attempted to launch due to \
                                 * an error earlier in the                       \
                                 * launch procedure                              \
                                 */
#define PRTE_JOB_STATE_ABORT_ORDERED                                                       \
    (PRTE_JOB_STATE_ERROR + 11) /* the processes in this job have been ordered to "die",   \
                                 * but may not have completed it yet. Don't order it again \
                                 */
#define PRTE_JOB_STATE_NON_ZERO_TERM \
    (PRTE_JOB_STATE_ERROR + 12) /* at least one process exited with non-zero status */
#define PRTE_JOB_STATE_FAILED_TO_LAUNCH (PRTE_JOB_STATE_ERROR + 13)
#define PRTE_JOB_STATE_FORCED_EXIT      (PRTE_JOB_STATE_ERROR + 14)
#define PRTE_JOB_STATE_SILENT_ABORT \
    (PRTE_JOB_STATE_ERROR           \
     + 16) /* an error occurred and was reported elsewhere, so error out quietly */

#define PRTE_JOB_STATE_REPORT_PROGRESS \
    (PRTE_JOB_STATE_ERROR + 17) /* report launch progress - not an error */
#define PRTE_JOB_STATE_ALLOC_FAILED \
    (PRTE_JOB_STATE_ERROR + 18) /* job failed to obtain an allocation */
#define PRTE_JOB_STATE_MAP_FAILED (PRTE_JOB_STATE_ERROR + 19) /* job failed to map */
#define PRTE_JOB_STATE_CANNOT_LAUNCH \
    (PRTE_JOB_STATE_ERROR + 20) /* resources were busy and so the job cannot be launched */
 ...
 * job states
 */
#define PRTE_JOB_STATE_DYNAMIC 100
